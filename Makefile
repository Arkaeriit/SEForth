...
FRT_SRC := core_forth_words.frt file_forth_func.frt string_forth_func.frt tools_forth_words.frt arg_and_exit_code_forth_words.frt
...
%.c : %.frt
	...
	cat $< | sed 's:[^.]( [^)]*): :g; s:^(.*)$$::; s:\s\+\([^"]\): \1:g; s:\\ .*::;  s:":\\":g; s:^:":; s:$$:\\n":;' | grep -v '" \?\\n"'  >> $@
	echo ';' >> $@
